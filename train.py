...
# -----------------------------------------------------------------------------
# 1. Turbo Transforms (512px for speed)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. Dataset (Optimized with OpenCV)
# -----------------------------------------------------------------------------
class VistaDataset(torch.utils.data.Dataset):
    def __init__(self, root, split='train', transforms=None, use_mosaic=False, img_size=512, limit=None):
        self.root, self.split, self.transforms = root, split, transforms
        self.use_mosaic, self.img_size = use_mosaic, img_size
        ...
        if limit: # Subsample for fast validation
            random.seed(42)
...
def calculate_metrics(preds, targets):
    ...
    for p, t in zip(preds, targets):
        if len(t['boxes']) == 0 or len(p['boxes']) == 0: continue
        ...
        total_acc += correct.float().mean().item(); count += 1
...
def main():
    ...
    print(f"Turbo Mode: Model={args.model} | ImgSize={args.img_size} | ValLimit=500")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with torch.no_grad():
            for images, targets in val_loader:
                images = [img.to(device) for img in images]
                total_val_acc += calculate_metrics(model(images), targets); val_count += 1
